...
export function Projects() {
  ...
  return (
    <section id="projects" className="bg-[#0e0e0e] text-stone-100 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-[1400px] px-6 lg:px-10 pt-20 lg:pt-28 pb-20 lg:pb-28">
        <h2 className="font-light tracking-[-0.02em] text-stone-50 text-5xl md:text-6xl lg:text-7xl">
          Case Studies
          <sup className="text-xs align-super text-stone-500 ml-1 tracking-normal">[02]</sup>
        </h2>

        {/* Project list */}
        <div className="mt-16 lg:mt-20 border-t border-stone-800/80">
          {projects.map((p, i) => (
            <a
              key={p.title}
              href="#"
              ...
              className={cn(
                "group flex items-center justify-between gap-6 py-7 lg:py-9 border-b border-stone-800/80 transition-colors",
            ...
            >
              <div className="flex items-center gap-5 lg:gap-7 min-w-0">
                ...
                <div
                  className={cn(
                    "shrink-0 w-16 lg:w-24 h-9 lg:h-12 bg-gradient-to-br transition-all duration-500",
                    p.gradient,
                    hovered === i ? "opacity-100 scale-100" : "opacity-50 scale-95"
                  )}
                />
                <h3 className="font-light tracking-[-0.02em] text-3xl md:text-5xl lg:text-6xl truncate">
                  {p.title}
                </h3>
              </div>
        ...
        {/* Clients grid */}
        <div className="mt-20 lg:mt-28 grid grid-cols-3 md:grid-cols-5 gap-x-6 gap-y-10 items-center justify-items-center">
          {clients.map((c) => (
            <div
              key={c}
